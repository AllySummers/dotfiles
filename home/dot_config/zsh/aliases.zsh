...
export LSCOLORS="${LSCOLORS:-$LS_COLORS}"

if command -v gls >/dev/null 2>&1; then
  alias ls='gls --color=tty --almost-all --literal'
else
...
if command -v eza >/dev/null 2>&1; then
  alias lsperms='eza --color=auto --almost-all --long --sort=size --group-directories-first --no-quotes --octal-permissions --no-filesize --no-time --no-user'
fi
